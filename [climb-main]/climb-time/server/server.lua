...
CLIMB, CLIMBclient = nil, nil

if Config.Framework == 'climb' then
    local Proxy = module('climb', 'lib/Proxy')
    local Tunnel = module('climb', 'lib/Tunnel')
    CLIMB = Proxy.getInterface('CLIMB')
    CLIMBclient = Tunnel.getInterface('CLIMB', 'chat_commands')
end
...
RegisterCommand(Config.Command.OpenUI, function(source)
    ...
    if PermissionsCheck(_source) then
        TriggerClientEvent('climb-time:OpenUI', _source, self)
    else
        Notification(_source, 3, L('invalid_permissions'))
    end
end)

RegisterServerEvent('climb-time:SyncMe')
AddEventHandler('climb-time:SyncMe', function(instant)
    local _source = source
    local temp = json.decode(json.encode(self))
    temp.instanttime = true
    temp.instantweather = true
    TriggerClientEvent('climb-time:ForceUpdate', _source, temp)
end)

RegisterServerEvent('climb-time:ForceUpdate')
AddEventHandler('climb-time:ForceUpdate', function(data)
    ...
    if PermissionsCheck(_source) then
        ...
        TriggerClientEvent('climb-time:ForceUpdate', -1, data)
    else
...
local function LoadSettings()
    ...
    if Config.Framework ~= 'climb' or Config.Framework ~= 'aceperms' then
        Wait(2000)
        local temp = json.decode(json.encode(self))
        temp.instanttime = true
        temp.instantweather = true
        TriggerClientEvent('climb-time:ForceUpdate', -1, temp)
    end
end
    ...
    while true do
        ...
        if not self.freeze then
            ...
            if not HasTriggered then
                HasTriggered = true
                TimeCounter = 0
                TriggerClientEvent('climb-time:SyncTime', -1, {hours = self.hours, mins = self.mins})
            end
        end
                ...
                if #Group >= TimesChanged then
                    ...
                    for _, d in pairs(Group) do
                        if LastWeatherTable[d] == nil then
                            ...
                            self.weather = d
                            TriggerClientEvent('climb-time:SyncWeather', -1, {weather = self.weather, instantweather = self.instantweather})
...
RegisterServerEvent('climb-time:ToggleInstantChange:Time')
AddEventHandler('climb-time:ToggleInstantChange:Time', function(boolean)
    self.instanttime = boolean
end)

RegisterServerEvent('climb-time:ToggleInstantChange:Weather')
AddEventHandler('climb-time:ToggleInstantChange:Weather', function(boolean)
...
RegisterServerEvent('climb-time:SaveSettings')
AddEventHandler('climb-time:SaveSettings', function()
    SaveSettngs()
end)
...
AddEventHandler('txAdmin:events:scheduledRestart', function(eventData)
    if eventData.secondsRemaining == math.ceil(Config.TsunamiWarning_time*60) then
        ...
        self.tsunami = true
        TriggerClientEvent('climb-time:StartTsunamiCountdown', -1, true)
    end
end)

RegisterServerEvent('climb-time:StartTsunamiCountdown')
AddEventHandler('climb-time:StartTsunamiCountdown', function(boolean)
    if not Config.TsunamiWarning then return end
    self.tsunami = boolean
    TriggerClientEvent('climb-time:StartTsunamiCountdown', -1, boolean)
end)

function PermissionsCheck(source)
    if Config.Framework == 'climb' then
        for c, d in pairs(Config.Command.Perms[Config.Framework]) do
            if CLIMB.hasPermission({CLIMB.getUserId({source}), d}) then 
                return true
            end
        end
        return false
