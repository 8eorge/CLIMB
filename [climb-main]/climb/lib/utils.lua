...
function module(rsc, path) -- load a LUA resource file as module
    if path == nil then -- shortcut for climb, can omit the resource parameter
        path = rsc
        rsc = "climb"
    end
    local key = rsc .. path
    ...
    if modules[key] then -- cached module
        ...
        if f then
            ...
            if ar[1] then
                ...
                modules[key] = nil
                print("^0[^3CLIMB^0] error loading module " .. rsc .. "/" .. path .. ":" .. ar[2])
            end
        else
            print("^0[^3CLIMB^0] error parsing module " .. rsc .. "/" .. path .. ":" .. err)
        end
    end
end
