...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(CLIMBConfig.PlayerSavingTime)
    if IsPlayerPlaying(PlayerId()) and state_ready then
      local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
      CLIMBserver.updatePos({x,y,z})
      CLIMBserver.updateHealth({tCLIMB.getHealth()})
      CLIMBserver.updateArmour({GetPedArmour(PlayerPedId())})
      CLIMBserver.updateWeapons({tCLIMB.getWeapons()})
      CLIMBserver.updateCustomization({tCLIMB.getCustomization()})
    end
  end
end)
...
function tCLIMB.getWeaponTypes()
  return weapon_types
end

function tCLIMB.getWeapons()
...
function tCLIMB.giveWeapons(weapons,clear_before)
  local player = GetPlayerPed(-1)
...
function tCLIMB.getDrawables(part)
...
function tCLIMB.getDrawableTextures(part,drawable)
  local isprop, index = parse_part(part)
  if isprop then
...
function tCLIMB.getCustomization()
...
function tCLIMB.setCustomization(custom) -- indexed [drawable,texture,palette] components or props (p0...) plus .modelhash or .model
    ...
    if custom then
      ...
      if mhash ~= nil then
        ...
        if HasModelLoaded(mhash) then
          ...
          local weapons = tCLIMB.getWeapons()
          SetPlayerModel(PlayerId(), mhash)
          tCLIMB.giveWeapons(weapons,true)
...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(60000)
    if state_ready then
      local custom = tCLIMB.getCustomization()
      custom.model = nil
      custom.modelhash = nil
      tCLIMB.setCustomization(custom)
    end
  end
end)
--]]
