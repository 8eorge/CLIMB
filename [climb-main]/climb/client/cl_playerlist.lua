    ...
    while true do
        if IsUsingKeyboard(2) then
            if IsControlJustPressed(0, 212) then
                a = not a
                TriggerServerEvent("CLIMB:getPlayerListData")
                Wait(100)
...
RegisterNUICallback("closeCLIMBPlayerList",function(j, k)
    SetNuiFocus(false, false)
end)
AddEventHandler("CLIMB:onClientSpawn",function(h, l)
    if l then
        TriggerServerEvent("CLIMB:getPlayerListData")
    end
end)

RegisterNetEvent("CLIMB:gotFullPlayerListData",function(staff, police, nhs, lfb, hmp, civillians)
...
local u,v,x
RegisterNetEvent("CLIMB:playerListMetaUpdate",function(t)
...
function sendFullPlayerListData()
    ...
    SendNUIMessage({setServerMetaData = '<img src="playerlist_images/luna.png" align="top" width="20px",height="20px"><span class="staff">' ..tostring(H) .. "</span>"})
...
Citizen.CreateThread(function()
    while true do
        ...
        if u ~= nil and v ~= nil and x ~= nil then
            SetDiscordAppId(1134660238696521758)
