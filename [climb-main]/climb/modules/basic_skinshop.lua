    ...
    if user_id ~= nil then
        ...
        CLIMBclient.getCustomization(source, {}, function(old_custom)
            ...
            local ontexture = function(player, choice)
                ...
                CLIMBclient.setCustomization(source, {custom})
            end
            ...
            local ondrawable = function(player, choice, mod)
                if mod == 0 then -- tex variation
                    ...
                    CLIMBclient.getDrawableTextures(source, {parts[choice], drawable[1]}, function(n)
                        print(n)
                        textures[choice][2] = n
            ...
            for k, v in pairs(parts) do -- for each part, get number of drawables and build menu
                ...
                -- get max drawables
                CLIMBclient.getDrawables(source, {v}, function(n)
                    drawables[k][2] = n -- set max
                   
                end)

                -- get max textures for this drawable
                CLIMBclient.getDrawableTextures(source, {v, drawables[k][1]}, function(n)
            ...
            menudata.onclose = function(player)
                ...
                CLIMBclient.getCustomization(source, {}, function(custom)
                    ...
                    if CLIMB.tryPayment(user_id, price) then
                        if price > 0 then
                            CLIMBclient.notify(source, {lang.money.paid({price})})
                        end
                    else
                        CLIMBclient.notify(source, {lang.money.not_enough()})
                        -- revert changes
                        CLIMBclient.setCustomization(source, {old_custom})
                    end
                end)
            end
            -- open menu
        ...
        for k, v in pairs(skinshops) do
            ...
            local skinshop_enter = function(source)
                ...
                if user_id ~= nil then
                    ...
                    CLIMB.openSkinshop(source, parts)
                end
            end
            ...
            CLIMBclient.addBlip(source, {x, y, z, 73, 3, lang.skinshop.title()})
            CLIMBclient.addMarker(source, {x, y, z - 1, 0.7, 0.7, 0.5, 0, 255, 125, 125, 150})

            CLIMB.setArea(source, "CLIMB:skinshop" .. k, x, y, z, 1, 1.5, skinshop_enter, skinshop_leave)
        end
    end
end
